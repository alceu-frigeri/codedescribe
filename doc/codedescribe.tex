...
\usepackage[strict,infograb,silence,index]{codedescribe}
\usepackage{enumitem}
...
\describe{index}{(new: 2025/12/15) This will set the default of some object groups to \tsobj[key,no index]{index}. This won't load any index package, but just change some objects' default behaviour (see \ref{indexing} and \ref{obj-type-def})}
\describe{index specs}{(new: 2025/12/19) This allows to change \tsobj[pkg]{makeindex} character specifiers. This expects a set of 4 parameters (from \tsobj[pkg]{makefile}: \tsobj[key]{level, actual, encap, quote}), for example, the default is \tsobj[key]{index specs} = \tsverb[value]{{{!}{@}{|}{"}}}}
...
Similarly, given the many index package variants (specially how index entries shall be created), the user is expected to supply an index generating command (key \tsobj[key]{index fmt}, see \ref{format-keys}), which shall absorb 3 parameters. This  \emph{user supplied} command will be used by 
...
\begin{codesyntax} %
	\tsmacro{\indexfmtraw}{prefix,group,item}
	\tsmacro{\indexfmtrawat}{prefix,group,item}
	\tsmacro{\indexfmtcsraw}{prefix,group,item}
	\tsmacro{\indexfmtcsrawat}{prefix,group,item}
\end{codesyntax}
\tsobj[code,no index]{\indexfmtraw{prefix}{group}{item}} is equivalent to \tsobj[code,no index]{\index{item}}. 

\tsobj[code,no index]{\indexfmtcsraw{prefix}{group}{\cmd}} is equivalent to \tsobj[code,no index]{\index{\string\cmd}}. That is, the first two parameters will be discarded and, in the case of \tsobj{\indexfmtcsraw}, the primitive \tsobj{\string} will precede the \tsobj[marg]{item} (if it is a command).

The other two commands, \tsobj[code,no index]{\indexfmtrawat,\indexfmtcsrawat},  will create index entries as \textbf{\tsmeta{prefix}\tsmeta{item}@\tsmeta{group}!\tsmeta{item}}. The backslash, if any, is removed from the first \tsmeta{item} (preceding the @) in the \tsobj[code,no index]{\indexfmtcsrawat}.
\end{codedescribe}
\begin{tsremark}
...
\end{tsremark}
\begin{tsremark}
When defining an Object Type (see below) only \tsobj[meta]{group} can be preset. \tsobj[meta]{prefix} can only be set when calling \tsobj[code,no index]{\tsobj} or using the \tsobj[env,no index,or]{codedescribe,describelist} environments.
\end{tsremark}
...
\begin{describelist*}{keys}
  ...
  \describe {index fmt} {Sets the `index generating' command (see \ref{indexing}) which shall absorb 3 parameters, like \tsobj[code,no index]{\usercmd{prefix}{group}{item}}. \tsobj[marg]{prefix} will come from the key \tsobj[key]{index prefix}, \tsobj[marg]{group} from the key \tsobj[key]{index group} and \tsobj[marg]{item} will be the item to be indexed.}
  \begin{tsremark}[\color{red}Important:]
    Except for \tsobj[key]{font,fsize,index fmt} all other keys will expand the assigned value at definition time!
  \end{tsremark}
\end{describelist*}
...
\begin{describelist*}{keys}
  ...
  \describe {code} {which sets \tsobj[keys]{code,color,index} (\tsobj[keys]{index fmt} = \tsobj{\indexfmtcsrawat})}
...
\begin{describelist*}{key}
  ...
  \describe {no index} {To NOT include the items in the default index}
  \describe {index} {To include the items in the default index}
  \describe {index group} {Sets the \tsobj[meta]{group} for those items (see \ref{indexing}).}
  \describe {index prefix} {Sets the \tsobj[meta]{prefix} for those items (see \ref{indexing}).}  
