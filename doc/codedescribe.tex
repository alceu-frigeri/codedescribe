...
%\usepackage[strict,infograb,silence,color scheme=darker,listing color scheme=darker]{codedescribe}
...
This package aims at documenting both document level (i.e. final user)  and package/class level commands. It's fully implemented using \tsobj[pkg]{expl3}, requiring just an up to date kernel. \tsobj[pkg]{scontents,listing} packages (see \cite{SCONTENTS} and \cite{listings}) are used to typeset code snippets. The package \tsobj[pkg]{pifont} is needed just to typeset those (open)stars, in case one wants to mark a command as (restricted) expandable. \tsobj[pkg]{xcolor} is needed to switch colors, and \tsobj[pkg]{xpeekahead} for spacing fine tuning. 
...
Generating an index of is supported (see \ref{indexing} and \ref{obj-types}) but but no index package is pre-loaded, leaving to it to the end user.
...
\describe{silence}{(new: 2025/11/22, defaults to 18.89999pt) This will suppress some annoying bad boxes warnings. Given the way environments at \ref{desc-env} are defined, with \tsobj[pkg]{expl} coffins, TeX sometimes thinks they are too wide, when they are not. This just sets \tsobj[code,no index]{\hfuzz}.}
...
\begin{describelist*}{keys}
  ...
  \describe {index group} {(defaults to `blank') sets the `index group' for those items (see \ref{indexing}).}
\end{describelist*}
...
\begin{describelist*}{key}
  ...
  \describe {index group} {(defaults to `blank') sets the \tsobj[meta]{index group} for those items (see \ref{indexing}).}
  \describe {index prefix} {(defaults to `blank') sets the \tsobj[meta]{index prefix} for those items (see \ref{indexing}).}  
\end{describelist*}
\end{codedescribe}
