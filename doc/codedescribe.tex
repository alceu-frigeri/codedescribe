...
\setnewcodekey{codedesc}{
  texcs2={defgroupfmt,defobjectfmt,tsmacro,tsobj},
...
No other package/class is needed, \st{any}, err, most classes can be used as the base one, which allows to demonstrate the documented commands with any desired layout. One glaring counter-example being \tsobj[pkg]{l3doc.cls} which defines an environment also named \tsobj[env]{tsremark}. It still can be used, BUT, the \tsobj[env]{tsremark} will get redefined (a warning will be issued in this case).
...
This doc regards to \tsobj[pkg]{codedescribe} version \PkgInfo{codedescribe}{version}, \tsobj[pkg]{codelisting} version \PkgInfo{codelisting}{version} and \tsobj[pkg]{codelstlang} version \PkgInfo{codelstlang}{version}. Those packages are fairly stable, and given the \tsobj[meta]{obj-type} mechanism (see \ref{obj-type-def}) they can be easily extended without changing their interface.
