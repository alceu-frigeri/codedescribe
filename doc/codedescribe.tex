...
%%
%% This is version {1.10} {2025/05/29}
%%
...
The applied format is defined in terms of \tsobj[arg]{obj-types}, which are defined in terms of \tsobj[arg]{format-groups} and each one defines a 'formatting function', 'font shape', bracketing, etc. to be applied.

\subsubsection{Format Keys}\label{format-keys}
Those are the primitive \tsobj[arg]{format-keys} used when defining  \tsobj[arg]{format-groups,obj-types} (see \ref{format-custom}):
...
\begin{describelist*}{key}
  ...
  \describe{bnf or}{To produce a bnf style or list, like \tsobj[option,bnf or,note={X and Z}]{abc,xdh,htf,hrf}.}
  \describe{meta or}{To produce a bnf style or list between angles, like \tsobj[option,meta or,note={X and Z}]{abc,xdh,htf,hrf}.}
\end{describelist*}
\end{codedescribe}
