...
\RequirePackage[verbose,a4paper,marginparwidth=27.5mm,top=2cm,bottom=1.5cm,hmargin={40mm,20mm},marginparsep=2.5mm,columnsep=10mm,asymmetric]{geometry}
...
One has the option to setting \tsmeta{code-keys} (see \ref{code-keys}) per \tsmacro{\tscode}{}/\tsmacro{\tsdemo}{} call, or \emph{globally}, better said, \emph{in the called context group} .\\
\begin{tsremark}[N.B.:]
...
\subsubsection{\tsobj[oarg]{format-keys}}\label{format-keys}
There is a set of primitive \tsobj[key]{format-keys} used to define a \tsobj[arg]{format-group} / \tsobj[arg]{obj-type}, which are:
...
\begin{describelist*}{keys}
  ...
  \describe {lbracket}  { defines the left bracket (when using \tsobj{\tsargs}),}
  \describe {rbracket}  { defines the right bracket (when using \tsobj{\tsargs}),}
...
\subsubsection{\tsobj[oarg]{format-groups}}
Based on the \tsobj[key]{format-keys} the following \tsobj[key]{format-groups} are pre-defined:
...
\subsubsection{\tsobj[oarg]{obj-types}}
Finally, based on those \tsobj[key]{format-groups} the \tsobj[key]{obj-type} keys are pre-defined:
...
\begin{codedescribe}[code,new=2023/05/17]{\defgroupfmt}
\begin{codesyntax} %
	\tsmacro{\defgroupfmt}{format-group,format-keys}
\end{codesyntax}
\tsobj[marg]{format-group} is the name of the new group (or one of the standard ones). \tsobj[marg]{format-keys} is any combination of the keys defined in \ref{format-keys}

\end{codedescribe}
For example, one can redefine the \tsobj[key]{code} standard color with \tsobj{\defgroupfmt{code}{color=red}} and all \tsobj[key]{obj-types} based on it will be typeset in red (in the standard case: \tsobj[key]{code, macro, function}).


\begin{codedescribe}[code,new=2023/05/17]{\defobjectfmt}
\begin{codesyntax} %
	\tsmacro{\defobjectfmt}{obj-type,format-group,format-keys}
\end{codesyntax}
\tsobj[marg]{obj-type} is the name of the new \tsobj[arg]{object} being defined (or redefined, in case of one already known/defined),
