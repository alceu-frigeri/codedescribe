...
%%
%% This is version {1.21-pre3} {2025/12/17}
%%
...
This package is designed to be as class independent as possible, depending only on \tsobj[pkg]{expl3,scontents,listing,xpeekahead,pifont,xcolor}. Unlike other packages of the kind, a minimal set of macros/commands/environments is defined: most/all defined commands have an ``object type'' as a \tsobj[key,no index]{keyval} parameter, allowing for an easy expansion mechanism (instead of the usual ``one set of macros/environments'' for each object type).

No assumption is made about page layout (besides ``having a margin paragraph''),  or underlying macros, so it should be possible to use these with any/most document classes.
...
This package aims at documenting both document level (i.e. final user)  and package/class level commands. It's fully implemented using \tsobj[pkg]{expl3}, requiring just an up to date kernel. \tsobj[pkg]{scontents,listing} packages (see \cite{SCONTENTS} and \cite{listings}) are used to typeset code snippets. 
...
\tsobj[pkg]{xcolor} (see \cite{xcolor}) is needed to switch colors, and \tsobj[pkg]{xpeekahead} (see \cite{xpeekahead}) for spacing fine tuning. The package \tsobj[pkg]{infograb} (see \cite{pkginfograb}) is also loaded, for package's documentation only (including  package's version tracking).
...
It loads: \tsobj[pkg]{listings,scontents,xpeekahead},  defines an environment: \tsobj[env,no index]{codestore} and a few commands for listing/demo code.
...
Thanks to \tsobj[pkg]{scontents}, it's possible to store \LaTeX{} code snippets in a \tsobj[pkg]{expl3} sequence variable.
...
\end{codesyntax}
These define sets of \LaTeX{} commands (csnames, sans the preceding slash bar), the \tsobj[key,no index]{set} variants initialize/redefine those sets (an empty list, clears the set), while the others extend those sets. The \tsobj[key,no index]{style} ones redefines the command display style (an empty \tsobj[meta]{value} resets the style to it's default).
\end{codedescribe}
...
\begin{codesyntax} %
	\tsobj[key]{numbers,numberstyle,firstnumber}
\end{codesyntax}
\tsobj[key]{numbers} possible values are \tsobj[value]{none} (default) and \tsobj[value]{left} (to add tinny numbers to the left of the listing). With \tsobj[key]{numberstyle} one can redefine the numbering style. \tsobj[key]{firstnumber} sets the numbering start, it can be any number, \tsobj[value,or]{last,auto}. It defaults to \tsobj[value]{last} (see \cite{listings} for details). 
\end{codedescribe}
...
\begin{codesyntax} %
	\tsobj[key]{parindent}
\end{codesyntax}
Sets the indentation to be used when `demonstrating' \LaTeX{}code (\tsobj[code]{\tsdemo}). Defaults to whatever value \tsobj[code,no index]{\parindent} was when the package was first loaded.
\end{codedescribe}
...
\begin{codesyntax} %
	\tsobj[key]{ruleht}
\end{codesyntax}
When typesetting the `code demo' (\tsobj{\tsdemo}) a set of rules are drawn. The Default, 1, equals to \tsobj[code,no index]{\arrayrulewidth} (usually 0.4pt).
\end{codedescribe}
...
\describe{describe keys}{(defaults to \tsobj[value]{group}) This sets the way the keys \tsobj[key]{new,update,note} are listed in a \tsobj[env]{codedescribe} environment, see \ref{desc-env}. Possible values are \tsobj[value,or]{group,as is, in sequence}. By default keys are grouped together, with \tsobj[value,or]{as is, in sequence} keys will respect the used sequence. }
...
\describe{index cmd}{(new: 2025/12/20) In case the index package being used defines a distinct index command. This set's the actual index command, defaults to \tsobj[code,no index]{index} (note, this is the cs name sans the backslash),  used by the provided auxiliary index commands (see \ref{indexing}). The given command must adhere to the same syntax of the original \tsobj[code,no index]{\index} command. }
\describe{index specs}{(new: 2025/12/19) This allows to change \tsobj[pkg]{makeindex} character specifiers. This expects a set of 4 parameters (from \tsobj[pkg]{makefile}: \tsobj[key]{level, actual, encap, quote}). Its default is \tsobj[key]{index specs} = \tsverb[value]{{{!}{@}{|}{"}}}}
\describe{index specs oc}{(new: 2025/12/19) This allows to change \tsobj[pkg]{makeindex} open/close character specifiers. It expects a set of 4 parameters (from \tsobj[pkg]{makefile}: \tsobj[key]{arg open, arg close, range open, range close}). Its default is \tsobj[key]{index specs oc} =  \tsverb[value]{ {{\{}{\}}{(}{)}} }. This isn't used, and is just a place holder in case further customization (indexes) is needed. }
\describe{index specs others}{(new: 2025/12/19) This allows to change \tsobj[pkg]{makeindex} others  specifiers. It expects a set of 3 parameters (from \tsobj[pkg]{makefile}: \tsobj[key]{escape, page compositor, index command}). Its default is \tsobj[key]{index specs others} = \tsverb[value]{{{\\}{-}{\indexentry}}}. This isn't used, and is just a place holder in case further customization (indexes) is needed. }
...
\describe{infograb}{This will enable the document level, \LaTeX2e, aliases from the package \tsobj[pkg]{pckinfograb} \cite{pkginfograb}.}
\begin{tsremark}
...
\end{codedescribe}
\begin{tsremark}
  The actual characters specifiers can be changed with the package option \tsobj[option]{index specs}, see \ref{codedesc.pack-options}.
\end{tsremark}
\begin{tsremark}
When defining an Object Type (see below) only \tsobj[meta]{group,name} can be preset. \tsobj[meta]{prefix} can only be set when calling \tsobj[code,no index]{\tsobj} or using the \tsobj[env,no index,or]{codedescribe,describelist} environments.
\end{tsremark}
\begin{tsremark}
Of course, \tsobj[marg]{name} is useful only in case of packages like \tsobj[pkg,or]{imakeidx,splitidx}, which allows more than one index file. Don't use/set \tsobj[key]{index name}, if you aren't using a multi-index aware package.
\end{tsremark}
...
\tsobj[arg]{obj-types} defines the applied format, which is defined in terms of \tsobj[arg]{format-groups}. Both define the formatting function, font shape, bracketing, etc. to be applied. When using a \tsobj[arg]{obj-type}, first the associated \tsobj[arg]{format-group} is applied, then the particular (if any) object format is applied.
...
\begin{describelist*}{keys}
  ...
  \describe {index fmt} {Sets the index generating command (see \ref{indexing}) which shall absorb 4 parameters, like \tsobj[code,no index]{\usercmd{name}{prefix}{group}{item}}.  \tsobj[marg]{prefix} will come from the key \tsobj[key]{index prefix}, \tsobj[marg]{group} from the key \tsobj[key]{index group} and \tsobj[marg]{item} will be the item to be indexed. \tsobj[marg]{name} will come from the key \tsobj[key]{index name} (if not empty,  \tsobj[marg]{name} will be between brackets). 
  
  For instance, having \tsobj[key]{index fmt} = \tsverb[value]{\usercmd}, \tsverb[code]{\tsobj[index name=iname, index prefix=pre, index group=grp]{\some}} will result in
  \tsverb[code]{\usercmd {[iname]} {pre}{grp}{\some}} being called/executed.
  }
  \begin{tsremark}[\color{red}Important:]
    Except for \tsobj[key]{font,fsize,index fmt} all other keys will be expanded at definition time!
  \end{tsremark}
\end{describelist*}
...
\begin{describelist*}{keys}
  ...
  \describe{args}{(default: gray) Sets the args base color}
  ...
  \describe{brightness}{(default: 1) Sets the mixing proportion between each base color and black.}
  \describe{default}{Sets all the above to their default value}
  \describe{scheme}{(no default) Selects a pre-set color scheme. see below} 
\end{describelist*}
...
\begin{describelist*}{key}
  ...
  \describe {no index} {To NOT include the items in the default index}
  \describe {index} {To include the items in the default index}
  \describe {index name} {Sets  \tsobj[meta]{name} for those items (see \ref{indexing}).}
  \describe {index group} {Sets  \tsobj[meta]{group} for those items (see \ref{indexing}).}
  \describe {index prefix} {Sets  \tsobj[meta]{prefix} for those items (see \ref{indexing}).}  
...
\end{tsremark}
\begin{tsremark}
  The default behaviour, when using \tsobj[key,no index,or]{new,update,note}, is that they will be grouped, first all \tsobj[key,no index]{new} keys, then all \tsobj[key,no index]{update} keys and lastly all \tsobj[key,no index]{note} keys. This can be changed with the package option \tsobj[option]{describe keys} (see \ref{codedesc.pack-options}). If set to \tsobj[value,or]{as is, in sequence}, those keys will be listed in the order of appearance.
\end{tsremark}
...
\tsobj[marg]{obj-type} defines the object-type format used to typeset \tsobj[marg]{item-name}, it can be any object from \ref{obj-types} (or \ref{format-custom}) and the following keys:
\begin{describelist*}{key}
  \describe {no index} {To NOT include {item names} in the default index.}
  \describe {index} {To include {item names} in the default index.}
  \describe {index name} {Sets  \tsobj[meta]{name} for those items (see \ref{indexing}).}  
  \describe {index group} {Sets  \tsobj[meta]{group} for those items (see \ref{indexing}).}
  \describe {index prefix} {Sets  \tsobj[meta]{prefix} for those items (see \ref{indexing}).}  
\end{describelist*}
...
\tsmacro{\describe}{item-name,item-description}
\end{codesyntax}
This is the \tsobj[env]{describelist} companion macro. In case of the \tsobj[env]{describe*}, \tsobj[marg]{item-name} will be typeset in a box \tsobj[oarg]{item-indent} wide, so that \tsobj[marg]{item-description} will be fully indented, otherwise \tsobj[marg]{item-name} will be typed in the margin.
\end{codedescribe}
...
%\begin{codedescribe}[code,new=2023/05/16,index group=Test]{\typesetdate,\tsdate}
%\begin{codesyntax*}[texcs4=tsmacro]
%\tsmacro{\typesetdate}{}
%\tsmacro{\tsdate}{}
%\end{codesyntax*}
%This provides the current date (in Month Year format).
