...
\usepackage[strict,infograb,load xtra dialects]{codedescribe}
%\usepackage[strict,infograb]{codedescribe}
\usepackage{enumitem}
...
\tstitle{
  ...
  date={\tsdate},
  title={The codedescribe, codelisting and codelstlang Packages\break Version \PkgInfo{codedescribe}{version}}
  }
...
No other package/class is needed, and it should be possible to use these packages with most classes\footnote{If, by chance, you find a class with compatibility issues, just open an issue at \url{https://github.com/alceu-frigeri/codedescribe/issues} to see what can be done}, which allows to demonstrate document commands with any desired layout.
...
All packages (\tsobj[pkg]{codedescribe,codelisting,codelstlang,codedescsets}) share the same version, currently: \PkgInfo{codelstlang}{version}. Those packages are fairly stable, and given the \tsobj[meta]{obj-type} mechanism (see \ref{obj-type-def}) they can be easily extended without changing their interface.
...
\begin{codedescribe}[key,new=2025/11/20]{lststyle}
...
\end{codesyntax}
Those define sets of \LaTeX{} commands (csnames, sans the preceding slash bar), the \tsobj[key]{set} variants initialize/redefine those sets (an empty list, clears the set), while the others extend those sets. The \tsobj[key]{style} ones redefines the command display style (an empty \tsobj[meta]{value} resets the style to it's default).
\end{codedescribe}
...
\describe{label set}{(new: 2025/11/22) This allows to pre-select a set of labels, see \ref{labelset}}
...
The following commands allows to customize the many `labels' in use, in particular the sub-package \tsobj[pkg]{codedescsets} holds a few locale sets, the user is invited, if needed, to submit translations for a specific case/language via a PR (Push Request) at \url{https://github.com/alceu-frigeri/codedescribe}


\begin{codedescribe}[code,new=2025/11/22]{\setcodelabels,\newlabelset,\selectlabelset}\label{labelset}
\begin{codesyntax} %
	\tsmacro{\setcodelabels}{label-keys}
	\tsmacro{\newlabelset}{lang,label-keys}
	\tsmacro{\selectlabelset}{lang}
\end{codesyntax}
kkkk
\end{codedescribe}

The \tsobj[marg]{label-keys} can be:
\begin{describelist}{key}
  \describe{new}{(defaults to: `new'). It set's the `new' label used in the \tsobj[env]{codedescribe} environment.}
  \describe{update}{(defaults to: `updated'). It set's the `update' label used in the \tsobj[env]{codedescribe} environment.}
  \describe{note}{(defaults to: `NB'). It set's the `note' label used in the \tsobj[env]{codedescribe} environment.}
  \describe{and}{(defaults to: `and'). It set's the `and' label used in the many \tsobj{\tsobj} commands (hint: last item separator).}
  \describe{or}{(defaults to: `or'). It set's the `or' label used in the many \tsobj{\tsobj} commands (hint: last item separator).}
  \describe{months}{(defaults to: {January, February, March, April, May, June, July, August, September, October, November, December}). It set's the month list used by the \tsobj{\tsdate} command, see \ref{aux-commands}.}
\end{describelist}
...
For example, this sets a new label set for German. In fact, since this is defined in the \tsobj[pkg]{codedescsets} this label set can be used when loading this package, see \ref{codedesc.pack-options}.
