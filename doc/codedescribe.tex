...
Generating an index is supported (see \ref{indexing} and \ref{obj-types}) but no index package is pre-loaded, leaving it to the end user.
...
The command \tsobj[code,no index]{\tsobj} and environments \tsobj[env,no index]{codedescribe,describelist} create index entries with the command \tsobj[code,no index]{\index} based on the object type (see \ref{obj-type-def}, \ref{desc-env} and \ref{ts-commands}).
The many items will be grabbed as \textbf{\tsmeta{index prefix}\tsmeta{item}@\tsmeta{index group}!\tsmeta{item}}. That way \tsobj[meta]{index prefix} can be used to force items to be sorted together  and \tsobj[meta]{index group} can be used to group, indent, similar items. The backslash, if any, is removed from the first \tsmeta{item} (preceding the @). Note that, when defining an Object Type (see below) only \tsobj[meta]{index group} can be preset. \tsobj[meta]{index-prefix} can only be set when calling \tsobj[code,no index]{\tsobj} or using the \tsobj[env,no index,or]{codedescribe,describelist} environments.
\begin{tsremark}
...
\begin{describelist*}{keys}
  ...
  \describe {index group} {Sets the `index group' for those items (see \ref{indexing}).}
  \begin{tsremark}[\color{red}Important:]
    Except for \tsobj[key]{font,fsize} all other keys will expand the assigned value at definition time!
  \end{tsremark}
\end{describelist*}
...
\begin{describelist*}{keys}
  ...
  \describe {code} {which sets \tsobj[keys]{code,color,index} (\tsobj[keys]{index group} = ``\tsobj[value]{command(s)}'')}
  \describe {oarg} {which sets \tsobj[keys]{meta,color}}
  \describe {syntax} {which sets \tsobj[keys]{color}}
  \describe {env}  {which sets \tsobj[keys]{slshape,color,index} (\tsobj[keys]{index group} = ``\tsobj[value]{environment(s)}'')}
  \describe {pkg}  {which sets \tsobj[keys]{slshape,color}}
  \describe {option} {which sets \tsobj[keys]{color,index} (\tsobj[keys]{index group} = ``\tsobj[value]{option(s)}'')}
  \describe {keys} {which sets \tsobj[keys]{slshape,color,index} (\tsobj[keys]{index group} = ``\tsobj[value]{key(s)}'')}
...
\end{tsremark}
\begin{tsremark}
\tsobj[keys]{index,index group} will only be set if the option \tsobj[option]{index} was used when loading this package, see \ref{codedesc.pack-options}.
\end{tsremark}
...
\begin{describelist*}{key}
  ...
  \describe {no index} {To NOT include the items in the default index}
  \describe {index} {To include the items in the default index}
  \describe {index group} {Sets the \tsobj[meta]{index group} for those items (see \ref{indexing}).}
  \describe {index prefix} {Sets the \tsobj[meta]{index prefix} for those items (see \ref{indexing}).}  
...
\end{codesyntax}
 ...
 Sample text. Sample test.
  ...
  \end{tsremark}
 \end{codestore}
 \tsdemo[codedesc,codeprefix={},resultprefix={},keywd4=Sample]{demo.remark}
\end{codedescribe}
...
\begin{codedescribe}[code,index group=Test]{\typesettitle,\tstitle}
\begin{codesyntax}
...
\begin{codedescribe}[env,index group=Test]{tsabstract}
\begin{codesyntax}
