...
%%
%% This is version {1.21-pre2} {2025/12/17}
%%
...
\tsobj[pkg]{xcolor} (see \cite{xcolor}) is needed to switch colors, and \tsobj[pkg]{xpeekahead} (see \cite{xpeekahead}) for spacing fine tuning. The package \tsobj[pkg]{infograb} (see \cite{pkginfograb}) is also loaded, for package's documentation only (including tracking of package's version).
...
Its up to the user to choose a companion package to format and display index entries, though a very simple setup, using \tsobj[pkg]{xindex}'s defaults, could just be:
...
The other two commands, \tsobj[code,no index]{\indexfmtrawat,\indexfmtcsrawat},  will create index entries as \textbf{\tsmeta{prefix}\tsmeta{item}@\tsmeta{group}!\tsmeta{item}}. That way \tsobj[meta]{prefix} can be used to force items to be sorted together  and \tsobj[meta]{group} can be used to group, indent, similar items. The backslash, if any, is removed from the first \tsmeta{item} (preceding the @) in the \tsobj[code,no index]{\indexfmtcsrawat}.
\end{codedescribe}
